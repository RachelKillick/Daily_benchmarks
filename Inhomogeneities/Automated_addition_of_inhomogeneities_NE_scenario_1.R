# This script is the final version used on the smaller realisation of the North East:

# Decided on a 40 stat smooth 

library(mgcv) # Necessary for the prediction stage

load('newup_ne_comp_221014_relocations_full_smaller_40_less.RData') # The station data - complete with relocation options - could make this smaller by getting rid of some extraneous variables - but should be OK as is I think 
...
Predictset$Station=sort(rep(1:146,30680))

Predictset$noise=Predictset$pred5other-Predictset$lsmodel40
...
load('necoastlattimenolakes.RData')
necoastlat=necoastlattimenolakes[with(necoastlattimenolakes,order(Station)),]

TMEANl=necoastlat[necoastlat$Station!=103,]
...
Inhomogeneity=as.data.frame(matrix(nrow=1460,ncol=5))
...
Urbanisation=as.data.frame(matrix(nrow=146,ncol=3)) # Doesn't need to be bigger than 75 as each station can have a max of one inhomogeneity
...
#							stat$pwc[(p+1):(p+l2)]=amp2*stat$pwc[(p+1):(p+l2)]
						...
						# will work even if one of the other loops has already been called...
							...
							statval=mean((60-predict.gam(modelnecoast5gf3other,newdata=stat[(p-l2+1):(p-l2+10),],type="response"))-(stat[(p-l2+1):(p-l2+10),19])) # Get a starting value for the urbanisation IH - This 19 needs to be whichever column is lsmodel...
	...
	Predictsetf[Predictsetf$Station==i,21]=statnew-stat$noise # The 21 here needs to be reset to be whichever column is the
...
Predictsetf$Difference=Predictsetf$Inhomogeneous-Predictsetf$lsmodel40
...
Predictsetf1[is.na(Predictsetf1$TMEAN),21] <- NA # Need to make sure that Predictsetf actually HAS a TMEAN column for this to work!
Predictsetf1$Difference=Predictsetf1$Inhomogeneous-Predictsetf1$lsmodel40

save(Inhomogeneity,Predictsetf,Predictsetf1,Urbanisation,Urbanisation1,file="NE_world_1_241014.RData") 
...
save(Inhomogeneitysub1,file='Inhomogeneitysub1_NE_world_1_241014.RData')
