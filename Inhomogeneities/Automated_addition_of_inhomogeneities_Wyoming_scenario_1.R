...
load('newup_wy_comp_17714_relocations_full_smaller_2025.RData') # The station data - complete with relocation options - could make this smaller by getting rid of some extraneous variables - but should be OK as is I think 
# This is a dataframe with 2301000 rows and 20 columns
...
Predictset$noise=Predictset$pred5th1-Predictset$lsmodel25
...
load('wyupdatefeb29gam.RData') # Check in a different script that this is in the same order as Predictsetf

Predictsetf$TMEAN=wyupdatefeb29gam$TMEAN
...
Inhomogeneity=as.data.frame(matrix(nrow=750,ncol=5))
...
Urbanisation=as.data.frame(matrix(nrow=75,ncol=3)) # Doesn't need to be bigger than 75 as each station can have a max of one inhomogeneity
...
#							stat$pwc[(p+1):(p+l2)]=amp2*stat$pwc[(p+1):(p+l2)]
						...
						# will work even if one of the other loops has already been called...
							...
							statval=mean((60-predict.gam(modelwycoast5gf3th1n,newdata=stat[(p-l2+1):(p-l2+10),],type="response"))-(stat[(p-l2+1):(p-l2+10),19])) # Get a starting value for the urbanisation IH
...
Predictsetf$Difference=Predictsetf$Inhomogeneous-Predictsetf$lsmodel25
...
Predictsetf1$Difference=Predictsetf1$Inhomogeneous-Predictsetf1$lsmodel25

save(Inhomogeneity,Predictsetf,Predictsetf1,Urbanisation,Urbanisation1,file="Wyoming_smaller_inhomogeneities_231014_propogate_backwards.RData") 
...
save(Inhomogeneitysub1,file='Inhomogeneitysub1_smaller_231014.RData')
