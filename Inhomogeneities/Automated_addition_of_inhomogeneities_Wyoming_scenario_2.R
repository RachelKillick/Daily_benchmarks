# A simple script: - When rolling this out to full length series instead of quarter length series there will be a lot of 4017s that need to become 15340s

# Changes to make in this version:

# - Alter so that IHs can't occur in the last two years of the record - DONE (though urbanisation inhomogeneities can persist right through this time period)
# - Alter so it's for the more dense network
# - Implement missing data algorithm (if I make it missing to the same level as the data I had to work with this will be reasonably straightforward) - Actually think I'll do this in a separate document after it's all been run through this
...
load('newup_wy_comp_31014_relocations_full_denser_25_minus_stat122.RData') # The station data - Need to make sure I have the noise values stored in this dataframe that will need to be added on to the predictions 
...
Predictset$Station=sort(rep(1:158,30680))

Predictset$noise=Predictset$pred5th1-Predictset$lsmodel25a
...
Inhomogeneity=as.data.frame(matrix(nrow=1580,ncol=5))
...
Urbanisation=as.data.frame(matrix(nrow=158,ncol=3)) # Doesn't need to be bigger than 75 as each station can have a max of one inhomogeneity
...
#							stat$pwc[(p+1):(p+l2)]=amp2*stat$pwc[(p+1):(p+l2)]
						...
						# will work even if one of the other loops has already been called...
							...
							statval=mean((60-predict.gam(modelwycoast5gf3th1n,newdata=stat[(p-l2+1):(p-l2+10),],type="response"))-(stat[(p-l2+1):(p-l2+10),20])) # Get a starting value for the urbanisation IH
...
Predictsetf$Difference=Predictsetf$Inhomogeneous-Predictsetf$lsmodel25a

Predictsetf1=Predictsetf

load('Missing_data_in_world3_231014.RData')
...
Predictsetf1$Difference=Predictsetf1$Inhomogeneous-Predictsetf1$lsmodel25a

save(Inhomogeneity,Predictsetf,Predictsetf1,Urbanisation,Urbanisation1,file="Wyoming_denser_inhomogeneities_231014_propogate_backwards_all.RData") 
...
save(Inhomogeneitysub1, file='Inhomogeneitysub1_wyoming_denser_231014_all.RData') 
