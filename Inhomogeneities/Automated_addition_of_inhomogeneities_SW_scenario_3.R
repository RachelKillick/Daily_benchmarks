...
load('newupd2_for_SW_IH_scripts_241014.RData') # The station data - Need to make sure I have the noise values stored in this dataframe that will need to be added on to the predictions 
...
Predictset$noise=Predictset$preds5th1-Predictset$lsmodel35b
...
Predictsetf$Difference=Predictsetf$Inhomogeneous-Predictsetf$lsmodel35b
...
TMEANl=TMEANl[TMEANl$Station!=151,] # Getting rid of the two stats I don't have in my predicted smaller world

load('secoastlattime.RData') # So I can select how to make the data missing from these stats
secoastlat=secoastlattime[with(secoastlattime,order(Station)),]

TMEAN=as.data.frame(matrix(nrow=3405480,1))
TMEAN[1:2316340,1]=TMEANl$TMEAN

sstat=sort(sample(1:155,size=71,replace=FALSE))

for (i in 1:71){
	TMEAN[((i+150)*15340+1):((i+151)*15340),1]=secoastlat$TMEAN[secoastlat$Station==sstat[i]]
}

save(sstat,TMEAN,file='Missing_data_in_world3_SW_241014.RData')
...
Predictsetf1$Difference=Predictsetf1$Inhomogeneous-Predictsetf1$lsmodel35b

save(Inhomogeneity,Predictsetf,Predictsetf1,file="SW_world3_IHs.RData") 
...
save(Inhomogeneitysub1,Inhomogeneity1,file='Inhomogeneitysub1_SW_world3.RData') 
