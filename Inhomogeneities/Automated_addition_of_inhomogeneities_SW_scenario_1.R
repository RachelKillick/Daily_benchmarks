...
# Decided on a 35 stat smooth over a 40 stat smooth 
...
load('newup_sw_comp_131014_relocations_full_smaller_station_smoothing_without_suspect_less.RData')
 # The station data - complete with relocation options - could make this smaller by getting rid of some extraneous variables - but should be OK as is I think 
...
Predictset$Station=sort(rep(1:151,30680))

Predictset$noise=Predictset$preds5th1-Predictset$lsmodel35
...
load('swupdatefeb29gam.RData') 

TMEANl=swupdatefeb29gam[swupdatefeb29gam$Station!=64,]
TMEANl=TMEANl[TMEANl$Station!=65,]
TMEANl=TMEANl[TMEANl$Station!=120,]
TMEANl=TMEANl[TMEANl$Station!=143,]
TMEANl=TMEANl[TMEANl$Station!=148,]
TMEANl=TMEANl[TMEANl$Station!=151,] 
...
Inhomogeneity=as.data.frame(matrix(nrow=1510,ncol=5))
...
Urbanisation=as.data.frame(matrix(nrow=151,ncol=3)) # Doesn't need to be bigger than 75 as each station can have a max of one inhomogeneity
...
#							stat$pwc[(p+1):(p+l2)]=amp2*stat$pwc[(p+1):(p+l2)]
						...
						# will work even if one of the other loops has already been called...
							...
							statval=mean((60-predict.gam(modelswcoast5gf3th1,newdata=stat[(p-l2+1):(p-l2+10),],type="response"))-(stat[(p-l2+1):(p-l2+10),20])) # Get a starting value for the urbanisation IH - This 20 needs to be whichever column is lsmodel...
...
Predictsetf$Difference=Predictsetf$Inhomogeneous-Predictsetf$lsmodel35
...
Predictsetf1$Difference=Predictsetf1$Inhomogeneous-Predictsetf1$lsmodel35

save(Inhomogeneity,Predictsetf,Predictsetf1,Urbanisation,Urbanisation1,file="SW_world_1_241014.RData") 
...
save(Inhomogeneitysub1,Inhomogeneity1,file='Inhomogeneitysub1_SW_world_1_241014.RData')
