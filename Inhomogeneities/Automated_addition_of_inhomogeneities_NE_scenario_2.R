...
load('newupd2_for_NE_IH_scripts_241014.RData') # The station data - Need to make sure I have the noise values stored in this dataframe that will need to be added on to the predictions 
...
Predictset$noise=Predictset$pred5other-Predictset$lsmodel40a
...
Urbanisation=as.data.frame(matrix(nrow=207,ncol=3)) # Doesn't need to be bigger than 210 as each station can have a max of one inhomogeneity
...
#							stat$pwc[(p+1):(p+l2)]=amp2*stat$pwc[(p+1):(p+l2)]
						...
						# will work even if one of the other loops has already been called...
							...
							statval=mean((60-predict.gam(modelnecoast5gf3other,newdata=stat[(p-l2+1):(p-l2+10),],type="response"))-(stat[(p-l2+1):(p-l2+10),21])) # Get a starting value for the urbanisation IH - the 21 needs to be whatever column is lsmodel...
	...
	Predictsetf[Predictsetf$Station==i,24]=statnew-stat$noise # The 24 here needs to be reset to be whichever column is the
...
Predictsetf$Difference=Predictsetf$Inhomogeneous-Predictsetf$lsmodel40a

Predictsetf1=Predictsetf

load('Missing_data_in_world3_NE_241014.RData')
...
Predictsetf1$Difference=Predictsetf1$Inhomogeneous-Predictsetf1$lsmodel40a

save(Inhomogeneity,Predictsetf,Predictsetf1,Urbanisation,Urbanisation1,file="NE_world2_IHs_241014.RData") 
...
save(Inhomogeneitysub1, file='Inhomogeneitysub1_NE_world2_241014.RData') 
