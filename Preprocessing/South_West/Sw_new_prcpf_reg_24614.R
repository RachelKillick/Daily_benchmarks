...
swallprcpf8114 <- open.ncdf('prcpf_sw2114.nc') # This should be sea level pressure data on 2x2 grid squares for -82 to -66 (278 to 294) longitude and 40 to 50 latitude

x8114 <- get.var.ncdf(swallprcpf8114, "lat")          # coordinate variable = 40.9517 39.0470 37.1422 35.2375 33.3328 31.4281 (29.5234 - got rid of)
y8114 <- get.var.ncdf(swallprcpf8114, "lon")          #  coordinate variable = (234.375 - got rid of) 236.250 238.125 240.000 241.875 243.750 245.625 247.500 (249.375 - got rid of)
...
prcpfall8114 <- get.var.ncdf(swallprcpf8114) 	         # PRCPF for each day from 1st January 1970 to 31st December 2011 - a matrix with 6 columns and 7 rows 
...
prcpfinterpp8114a=data.frame(matrix(ncol=2,nrow=21169200))
...
call8114a=interpp(lat8114a,lon8114a,PRCPF8114a,swele$Lat,swele$Long, linear=TRUE, extrap=FALSE, duplicate = "error")

j=1380*(i-1)+1
j1=1380*i
prcpfinterpp8114a[j:j1,1]=call8114a$z
prcpfinterpp8114a[j:j1,2]=swele$Code
...
save(prcpfinterpp8114c,file='prcpf_sw24614.RData')

rm(prcpfinterpp8114a,prcpfinterpp8114b,prcpfinterpp8114c,call8114a,i,j,j1,lat8114a,lon8114a,swallprcpf8114,swele,SWregcoast1,prcpf8114,PRCPF8114a,prcpfall8114,x8114,y8114,yneg8114)
